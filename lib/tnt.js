...
class TNT {
    constructor({accounts, username, password, debug}) {
    ...
    async renderLabel(xml, options = {}) {
        ...
        const filename = uuid();
        const tempfilesPath = path.resolve(__dirname,`./tempfiles`);
        const currentTempFile = tempfilesPath + "/" + filename + ".xml";
        if (!fs.existsSync(tempfilesPath)){
            fs.mkdirSync(tempfilesPath);
        }
        await fs.writeFileSync( currentTempFile, xml, { mode: 0o755 });
        //xslt3 -xsl:HTMLRoutingLabelRenderer.xsl -export:HTMLRoutingLabelRenderer.sef.json -t -ns:##html5
        return await SaxonJS.transform({
            stylesheetFileName: path.resolve(__dirname, './label/HTMLRoutingLabelRenderer.sef.json'),
            sourceFileName: currentTempFile,
